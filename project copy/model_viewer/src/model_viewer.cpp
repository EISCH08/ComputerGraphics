...
//Parker Eischen
//export ASSIGNMENT3_ROOT=$HOME/Documents/GitHub/ComputerGraphics/project
...
struct Context {
    ...
    glm::vec3 diffuseColor = glm::vec3(1.0f, 0, 0.0f);
    bool diffuseOn = true;
    ...
    bool gammaOn = true;
    float zoomFactor;
    glm::vec3 lightPos = glm::vec3(0.0, 0.0, -30.0); // Position of the light
    ...
    int activeTexture;
    bool celToggle = false;
    int levels = 3;
    float edgeDetectionThreshold;
    float specMaskThreshold;

    bool noiseLines = false;
    bool noiseCubes = false;
    int cubeScaling = 5;
    float lineDensity = .5;
    bool noiseLava = false;
    int activeObject = 1;
...
void init(Context &ctx)
{
    ...
    std::string objects[5] = {"armadillo.obj", "bunny.obj", "gargo.obj", "icosphere.obj", "teapot.obj"};

    loadMesh((modelDir() + objects[ctx.activeObject]), &ctx.mesh);
...
void drawMesh(Context &ctx, GLuint program, const MeshVAO &meshVAO)
{
    ...
    glUniform1f(glGetUniformLocation(program, "u_edgeDetectionThreshold"), ctx.edgeDetectionThreshold);
    glUniform1f(glGetUniformLocation(program, "u_specMaskThreshold"), ctx.specMaskThreshold);
    glUniform1f(glGetUniformLocation(program, "u_lineDensity"), ctx.lineDensity);

    glUniform1i(glGetUniformLocation(program,"u_specularOn"),ctx.specularOn);
    glUniform1i(glGetUniformLocation(program,"u_celToggle"),ctx.celToggle);
    ...
    glUniform1i(glGetUniformLocation(program,"u_noiseCubes"),ctx.noiseCubes);
    glUniform1i(glGetUniformLocation(program,"u_noiseLines"),ctx.noiseLines);
    glUniform1i(glGetUniformLocation(program,"u_rgbSurfaceNormal"),ctx.rgbSurfaceNormal);
    glUniform1i(glGetUniformLocation(program,"u_levels"),ctx.levels);
    glUniform1i(glGetUniformLocation(program,"u_cubeScaling"),ctx.cubeScaling);
    glUniform1i(glGetUniformLocation(program,"u_noiseLava"),ctx.noiseLava);
...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(ctx.window)) {
        ...
        ImGui::Checkbox("Cel Shading Toggle", &ctx.celToggle);
        ImGui::SliderInt("Number of Levels", &ctx.levels, 1,10);
        ImGui::SliderFloat("Edge Detection Threshold", &ctx.edgeDetectionThreshold,0.0f,1.0f);
        ImGui::SliderFloat("Specular Mask Threshold", &ctx.specMaskThreshold,0.0f,1.0f);
        ...
        ImGui::SliderFloat3("Light Position", &ctx.lightPos[0],-30.0f,30.0f);    
        ImGui::Checkbox("Gamma Toggle", &ctx.gammaOn);
        ImGui::Checkbox("Noise Cube Toggle", &ctx.noiseCubes);
        ImGui::SliderInt("Cube Scale", &ctx.cubeScaling, 1,10);
        ImGui::Checkbox("Noise Line Toggle", &ctx.noiseLines);
        ImGui::SliderFloat("Line Density", &ctx.lineDensity,0.0f,1.0f);
        ImGui::Checkbox("Noise Lava Effect", &ctx.noiseLava);
